...
async function getAllFiles(dir, baseDir = dir) {
    ...
    const files = await Promise.all(
        entries.map(async (entry) => {
            ...
            if (entry.isDirectory() && (entry.name === 'node_modules' || entry.name === '.cdk')) {
                return [];
            }
...
async function syncLocalRepo(repoName) {
    ...
    try {
        ...
        console.log(`⚠️  Skipping ${repoName} - directory not found at ${targetRepoDir}`);
...
async function main() {
    ...
    for (const repo of reposToSync) {
        ...
        try {
            ...
            if (result.synced === 0 && result.created === 0) {
                ...
                console.log(`  ✓ Done: ${result.created} created, ${result.synced} updated`);
            }
        } catch (err) {
    ...
    const successful = Object.entries(results).filter(([_, r]) => !r.error && !r.skipped);
